  ...
  try {
    ...
    res.json(
      result.rows.map((row) => ({
        ...
        completedProcedures: row.completed_procedures ? (typeof row.completed_procedures === 'string' ? JSON.parse(row.completed_procedures) : row.completed_procedures) : null,
...
router.post('/consultation/:clinicId', async (req, res) => {
  try {
    ...
    res.status(201).json({
      ...
      completedProcedures: result.rows[0].completed_procedures ? (typeof result.rows[0].completed_procedures === 'string' ? JSON.parse(result.rows[0].completed_procedures) : result.rows[0].completed_procedures) : null,
    ...
    const result = await query(
      `UPDATE daily_consultation_entries
       SET date = $1, patient_name = $2, code = $3,
           consultation_type_id = $4, consultation_completed = $5, consultation_completed_at = $6, completed_procedures = $7,
           plan_created = $8, plan_created_at = $9,
           plan_presented = $10, plan_presented_at = $11, plan_presented_value = $12,
           plan_accepted = $13, plan_accepted_at = $14,
           plan_value = $15,
           source_id = $16, is_referral = $17, referral_name = $18, referral_code = $19, campaign_id = $20,
           doctor_id = $21,
           plan_not_eligible = $22, plan_not_eligible_at = $23, plan_not_eligible_reason = $24
       WHERE id = $25 AND clinic_id = $26
       RETURNING *`,
      [
        date,
        patientName,
