...
interface ProcedureBaseEditorProps {
  clinicId: string
  readOnly?: boolean
}
...
export function ProcedureBaseEditor({ clinicId, readOnly = false }: ProcedureBaseEditorProps) {
  ...
  const [importConfig, setImportConfig] = useState({
    isPericiable: false,
    category: '',
    useEncargo: true, // true = encargo_adse_eur, false = copagamento_beneficiario_eur
  })
  ...
  const loadProcedures = async () => {
    setLoading(true)
    try {
      const data = await advancesApi.proceduresBase.getAll(clinicId)
      setProcedures(data)
    } catch (err: any) {
  ...
  const handleAdd = async () => {
    ...
    setSaving(true)
    try {
      await advancesApi.proceduresBase.create(clinicId, {
        ...
        defaultValue: newProcedure.defaultValue ? parseFloat(newProcedure.defaultValue) : null,
      })
      toast.success('Procedimento adicionado com sucesso')
      setNewProcedure({ code: '', description: '', isPericiable: false, category: '', defaultValue: '' })
      loadProcedures()
    } catch (err: any) {
  ...
  const handleSaveEdit = async (id: string) => {
    ...
    setSaving(true)
    try {
      await advancesApi.proceduresBase.update(clinicId, id, {
        ...
        defaultValue: editProcedure.defaultValue ? parseFloat(editProcedure.defaultValue) : null,
  ...
  const handleDelete = async (id: string) => {
    ...
    setSaving(true)
    try {
      await advancesApi.proceduresBase.delete(clinicId, id)
  ...
  const handleExecuteImport = async () => {
    ...
    for (const item of importPreview) {
      try {
        // Converter valor (vírgula para ponto)
